    ...
    <div className="space-y-4">
      {isLoading ? (
        <div className="py-6 flex items-center justify-center gap-2">
          ...
          <span className="text-xs text-textMuted select-none">Loading audit trail...</span>
        </div>
      ) : activities.length === 0 ? (
        ...
        <div className="relative border-l border-borderSep ml-3.5 pl-5 space-y-5">
          {activities.map((activity, idx) => {
            ...
            return (
              <div key={activity._id || idx} className="relative group">
                ...
                <div
                  className="absolute -left-[30px] top-0.5 w-6 h-6 rounded-full border border-cardBg flex items-center justify-center text-[8px] font-extrabold text-white select-none shadow-premium ring-4 ring-cardBg"
                ...
                <div className="text-xs leading-relaxed">
                  <div className="text-textMuted">
                    <span className="font-semibold text-textPrimary mr-1.5 hover:text-accent hover:underline cursor-help" title={userObj.email}>
                      {userObj.name}
                    </span>
                    <span>{activity.details || activity.action}</span>
                  </div>
                  <span className="text-[9px] text-textMuted/60 mt-1.5 block select-none">
                    {timeString}
                  </span>
                </div>
